...
for ii = wind_simul
    ...
    progress = "Progresso da simulação "+string(ii/0.1 + 1);
    hbar = waitbar(0,progress);
    ...
    ise_array = cat(2,ise_array,ise);
    disp(ise);
    plotQuadrotorTrajectory;
    close(hbar);
end
figure('Name','Simulação do vento')
hold on
plot(wind_simul, ise_array)
grid on
ylabel('erro quadrático')
xlabel('velocidade do vento(m/s)')
title('Erro quadrático para dada velocidade do vento')
