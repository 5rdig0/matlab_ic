function animation = drone_Animation(xHistory, env)
...
%  drawnow
 ...
 %plot3(envelope(:,1),envelope(:,2),envelope(:,3), 'y:','Marker','o','MarkerSize', env*30);

function ring_plot = draw_ring(x, y, z, varx, vary, varz, size)
    teta = linspace(-pi, pi, 100);
    xx = size*cos(teta);
    yy = size*sin(teta);
    plot3(xx,yy,zeros(1,numel(xx)), 'b')
    drawnow
end
for i = 1:length(x)
    draw_ring(x(i), y(i), z(i), varx(i), vary(i), varz(i), env);
end
 for i = 1:length(x)

     plot3(x(1:i),y(1:i),z(1:i), 'g:','LineWidth',1.5);
